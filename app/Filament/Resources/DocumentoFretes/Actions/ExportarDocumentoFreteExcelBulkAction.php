...
class ExportarDocumentoFreteExcelBulkAction
{
    public static function make(): BulkAction
    {
        return BulkAction::make('exportar-excel')
            ...
            ->modalDescription(fn (Collection $records) => 'Você está prestes a exportar '.$records->count().' documento(s) de frete para Excel.'
            )
    ...
    protected static function exportToExcel(Collection $records): StreamedResponse
    {
        ...
        try {
            ...
            foreach ($records as $record) {
                $sheet->setCellValueExplicit('A'.$row, $record->id, DataType::TYPE_NUMERIC);
                ...
                $sheet->setCellValue('F'.$row, $record->data_emissao ? Carbon::parse($record->data_emissao)->format('d/m/Y') : '');
                ...
                $sheet->setCellValue('N'.$row, $record->created_at ? Carbon::parse($record->created_at)->format('d/m/Y H:i') : '');
                $sheet->setCellValue('O'.$row, $record->updated_at ? Carbon::parse($record->updated_at)->format('d/m/Y H:i') : '');
