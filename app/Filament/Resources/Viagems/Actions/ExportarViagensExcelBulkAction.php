...
class ExportarViagensExcelBulkAction
{
    public static function make(): BulkAction
    {
        return BulkAction::make('exportar-excel')
            ...
            ->modalDescription(fn (Collection $records) => 'Você está prestes a exportar '.$records->count().' viagen(s) para Excel.'
            )
    ...
    protected static function exportToExcel(Collection $records): StreamedResponse
    {
        ...
        try {
            ...
            foreach ($records as $record) {
                ...
                $sheet->setCellValue('M'.$row, $record->data_competencia ? Carbon::parse($record->data_competencia)->format('d/m/Y') : '');
                $sheet->setCellValue('N'.$row, $record->data_inicio ? Carbon::parse($record->data_inicio)->format('d/m/Y H:i') : '');
                $sheet->setCellValue('O'.$row, $record->data_fim ? Carbon::parse($record->data_fim)->format('d/m/Y H:i') : '');
