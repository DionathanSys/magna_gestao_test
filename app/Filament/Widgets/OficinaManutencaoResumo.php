...
class OficinaManutencaoResumo extends StatsOverviewWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $comparativo = $this->makeComparativoPeriodoStat();
    ...
    private function makeComparativoPeriodoStat(): Stat
    {
        [$currentStart, $currentEnd] = $this->resolveCurrentPeriod();

        if (! $currentStart || ! $currentEnd) {
            return Stat::make('Atual x Período Anterior', 'Sem dados')
        ...
        return Stat::make('Atual x Período Anterior', 'R$ '.number_format($currentTotal / 100, 2, ',', '.'))
            ->description(sprintf(
                '%s x %s | %s',
    ...
    private function resolveCurrentPeriod(): array
    {
        ...
        $resolvedEnd = Carbon::parse($latestDate)->endOfDay();
        $resolvedStart = Carbon::parse($latestDate)->startOfMonth()->startOfDay();
